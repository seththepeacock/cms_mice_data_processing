...
num_of_readings = num_of_readings / 2;

half_LGPD_processes = NaN(num_of_mice, max_number_of_pairs, 2);
half_RGPD_processes = NaN(num_of_mice, max_number_of_pairs, 2);
        ...
        if this_val > previous_val && this_val > next_val
            ...
            for m = i-1:-1:2
                ...
                if this_val < previous_val && this_val < next_val
                    ...
                    half_LGPD_processes(this_mouse, paired_maxes, 1) = nearest_left_min;
                    %add the max to output data
                    half_LGPD_processes(this_mouse, paired_maxes, 2) = found_max;
                    break
                end
        ...
        if this_val > previous_val && this_val > next_val
            ...
            for m = i+1:1:num_of_readings - 1
                ...
                if this_val < previous_val && this_val < next_val
                    ...
                    %add the min to output data
                    half_RGPD_processes(this_mouse, paired_maxes, 1) = nearest_right_min;
                    %add the max to output data
                    half_RGPD_processes(this_mouse, paired_maxes, 2) = found_max;
                    break
                end
...
half_LGPD_coordinates3D = NaN(num_of_mice, 3);

for this_mouse = 1:num_of_mice
    all_mins = half_LGPD_processes(this_mouse, 1:max_number_of_pairs, 1);
    all_maxes = half_LGPD_processes(this_mouse, 1:max_number_of_pairs, 2);
    all_persistences = all_maxes - all_mins;
    smallest_min = min(all_mins);
    largest_min = max(all_mins);
    max_persistence = max(all_persistences);
    half_LGPD_coordinates3D(this_mouse, 1) = smallest_min;
    half_LGPD_coordinates3D(this_mouse, 2) = largest_min;
    half_LGPD_coordinates3D(this_mouse, 3) = max_persistence;
end


%extract features for Right 3D
half_RGPD_coordinates_3D = NaN(num_of_mice, 3);

for this_mouse = 1:num_of_mice
    all_mins = half_RGPD_processes(this_mouse, 1:max_number_of_pairs, 1);
    all_maxes = half_RGPD_processes(this_mouse, 1:max_number_of_pairs, 2);
    all_persistences = all_maxes - all_mins;
    smallest_min = min(all_mins);
    largest_min = max(all_mins);
    max_persistence = max(all_persistences);
    half_RGPD_coordinates_3D(this_mouse, 1) = smallest_min;
    half_RGPD_coordinates_3D(this_mouse, 2) = largest_min;
    half_RGPD_coordinates_3D(this_mouse, 3) = max_persistence;
end



save half_GPD_processes.mat half_LGPD_processes half_LGPD_coordinates3D half_RGPD_processes half_RGPD_coordinates_3D
